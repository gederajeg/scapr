...
[![Build Status](https://travis-ci.org/gederajeg/scapr.svg?branch=master)](https://travis-ci.org/gederajeg/scapr)<br/>
[![codecov](https://codecov.io/gh/gederajeg/scapr/branch/master/graph/badge.svg)](https://codecov.io/gh/gederajeg/scapr)<br/>
...
We can use `scaps()` as an inline R code, e.g. to turn the word *happiness* into small caps, as follows: `` `r knitr::inline_expr('scapr::scaps("happiness")')` ``. It will then produce `r scaps("happiness")`.

Another example is to include another markdown formatting, such as **boldface** (e.g., `` `r knitr::inline_expr('scapr::scaps("**happiness**")')` `` that results in `r scaps("**happiness**")` and *italics* (e.g., `` `r knitr::inline_expr('scapr::scaps("*happiness*")')` `` that results in `r scaps("*happiness*")`.
